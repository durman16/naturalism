...
function updateFavBtn(btn, id) {
  ...
  btn.setAttribute('aria-label', isFav
    ? (t['ui.fav.remove'] || 'Favoriden çıkar')
    : (t['ui.fav.add'] || 'Favoriye ekle'));
  btn.querySelector('.fav-icon').textContent = isFav ? '♥' : '♡';
}

function updateFavButtons() {
  document.querySelectorAll('.fav-btn').forEach(btn => updateFavBtn(btn, btn.dataset.id));
}

/* ── share ── */
function shareCard(card, type) {
  const title = card.querySelector('h3,h4')?.textContent?.trim() || document.title;
  const base = window.location.href.split('#')[0].split('?')[0];
  const url = `${base}#${card.id}`;
  ...
  if (type === 'wa') {
    ...
    navigator.clipboard.writeText(`${title} — ${url}`).then(() => {
      const lbl = card.querySelector('[data-share="copy"] .share-label');
      if (!lbl) return;
      const orig = lbl.textContent;
      lbl.textContent = window.T[currentLang]['ui.share.copied'] || 'Kopyalandı!';
...
function injectCardActions() {
  ...
  cards.forEach((card, i) => {
    ...
    const favLabel = favorites.has(id)
      ? (t['ui.fav.remove'] || 'Favoriden çıkar')
      : (t['ui.fav.add']    || 'Favoriye ekle');
    ...
    actions.innerHTML = `
      <div class="share-group">
        ...
        <button class="share-btn" data-share="x"    aria-label="${xLabel}">
          <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.748l7.73-8.835L1.254 2.25H8.08l4.261 5.635 5.903-5.635zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
    ...
    /* herb-cards: wrap icon+info in .herb-card-top, then append actions below */
    const herbIcon = card.querySelector('.herb-icon');
    const herbInfo = card.querySelector('.herb-info');
    if (herbIcon && herbInfo && !card.querySelector('.herb-card-top')) {
      const top = document.createElement('div');
      top.className = 'herb-card-top';
      card.insertBefore(top, herbIcon);
      top.appendChild(herbIcon);
      top.appendChild(herbInfo);
    }
...
function buildDropdown(q) {
  ...
  document.querySelectorAll('.recipe-card, .herb-card').forEach(card => {
    const heading = card.querySelector('h3, h4');
    if (!heading) return;
    const title = heading.textContent.trim();
    if (normalize(title).includes(nq) || normalize(card.textContent).includes(nq)) {
      results.push({ title, id: card.id });
    }
  });

  if (!results.length) {
    dropdown.innerHTML = `<div class="dd-empty">${window.T[currentLang]['ui.no_results'] || 'Sonuç bulunamadı.'}</div>`;
    dropdown.hidden = false;
    return;
  }

  dropdown.innerHTML = results.map(r =>
    `<a class="dd-item" href="#${r.id}">${r.title}</a>`
  ).join('');

  dropdown.querySelectorAll('.dd-item').forEach(a => {
...
searchInput?.addEventListener('keydown', e => {
  if (e.key === 'Escape') { dropdown.hidden = true; searchInput.value = ''; }
});
...
function applyFilter() {
  ...
  cards.forEach(card => {
    const show = activeFilter !== 'fav' || favorites.has(card.id);
    card.style.display = show ? '' : 'none';
    if (show) visible++;
  });

  /* hide/show sections that have no visible cards */
  document.querySelectorAll('.section, #sifali-bitkiler').forEach(section => {
    const anyVisible = [...section.querySelectorAll('.recipe-card, .herb-card')]
  ...
  if (noRes) {
    ...
    if (!visible) {
      noRes.querySelector('span').textContent =
        activeFilter === 'fav'
          ? (window.T[currentLang]['ui.fav.empty']  || 'Henüz favori eklemediniz.')
          : (window.T[currentLang]['ui.no_results'] || 'Sonuç bulunamadı.');
    }
  }
}
